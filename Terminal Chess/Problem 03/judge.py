...
def condition_func_problem_03(test_func, case):
    return test_func(*case[0]) == case[1]

def generate_test_cases(n):
    test_case = [((1,1,1,1,1,1),0), ((10**9,)*6,0), ((10**9,)*4+(1,10**9),1), ((10**9,)*5+(1,),1), ((10**9,)*4+(1,1),1), ((5,)*6,0), ((5,)*4+(3,3),1), ((5,)*5+(3,),1), ((5,)*4+(3,5),1), ((5,)*4+(1,1),1), ((3*7*11*13*17*19*23*29*31*37+1,3*7*11*13*17*19*23*29*31*37+2,3*7*11*13*17*19*23*29*31+3, 3*7*11*13*17*19+4,3+5,7+6),2)]
    test_case += [((M:=random.randint(1,10**9),N:=random.randint(1,M),(x1:=random.randint(1,M)),(y1:=random.randint(1,N)),(x2:=random.randint(1,M)),(y2:=random.randint(1,N))),0 if x1==x2 and y1==y2 else 1 if x1==x2 or y1==y2 or abs(x1-x2)==abs(y1-y2) else 2) for _ in range(n-11)]
    return test_case

test(10, 1000, get_minimum_move, condition_func_problem_03)